...
async function drawTweetCard(ctx: PImage.Context, card: ServerRenderCard) {
  ...
  drawLeftAlignedLines(ctx, bodyLines, textX, bodyStartY, 80, bodyFontSize, textColor);
  currentY += bodyLines.length * 80;
  ...
  if (card.imageUrl) {
    ...
    if (tweetImg) {
      ...
      const imageWidth = Math.min(maxImageWidth, width - paddingX * 2);
